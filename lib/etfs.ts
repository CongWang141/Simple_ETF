...
const summarySelect = `SELECT e.symbol, e.isin, e.name, e.tracking_index AS trackingIndex, e.index_provider AS indexProvider, e.market, e.exchange, e.country, e.issuer, e.asset_class AS assetClass, e.region, e.industry, e.strategy, e.fund_currency AS fundCurrency, e.trading_currency AS tradingCurrency, e.domicile, e.inception_date AS inceptionDate, e.ter, e.fund_size_million_usd AS fundSizeMillionUsd, e.distribution_policy AS distributionPolicy, e.replication_method AS replicationMethod, e.benchmark_name AS benchmarkName, m.as_of_date AS asOfDate, m.pe_ratio AS peRatio, m.pb_ratio AS pbRatio, m.return_1m_pct AS return1mPct, m.return_3m_pct AS return3mPct, m.return_6m_pct AS return6mPct, m.return_ytd_pct AS returnYtdPct, m.return_1y_pct AS return1yPct, m.return_3y_pct AS return3yPct, m.return_5y_pct AS return5yPct, m.return_since_inception_pct AS returnSinceInceptionPct FROM etfs e JOIN etf_metrics m ON m.etf_id = e.id`;

export function listEtfs(filters: EtfFilters = {}): EtfSummary[] {
  const conditions: string[] = [];
  const parameters: unknown[] = [];
  const filtersToColumns: Array<[keyof EtfFilters, string]> = [["assetClass", "e.asset_class"], ["region", "e.region"], ["country", "e.country"], ["industry", "e.industry"], ["strategy", "e.strategy"], ["distributionPolicy", "e.distribution_policy"]];
...
export function getHistoricalObservations(symbol: string): HistoricalObservation[] {
  return db.prepare(`SELECT p.date, p.close_price AS closePrice, p.currency, r.daily_price_return_pct AS dailyPriceReturnPct, r.daily_total_return_pct AS dailyTotalReturnPct, r.total_return_index AS totalReturnIndex, r.cumulative_total_return_pct AS cumulativeTotalReturnPct FROM etfs e JOIN price_history p ON p.etf_id = e.id JOIN return_history r ON r.etf_id = e.id AND r.date = p.date WHERE e.symbol = ? ORDER BY p.date`).all(symbol) as HistoricalObservation[];
}
